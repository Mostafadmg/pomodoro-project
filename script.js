...
function handleHold(arrow, actionFn) {
  ...
  // Helper: start the hold
  const startHold = () => {
    const input = arrow.closest(".setting__time-input-group").querySelector("input");
    actionFn(input); // immediate step
    intervalId = setInterval(() => actionFn(input), 120); // repeating
  };

  // Helper: stop the hold
  const stopHold = () => {
  ...
  arrow.addEventListener("mouseout", stopHold);
  ...
  // Prevent normal click (important on mobile)
